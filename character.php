...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['head_color'])) {
        ...
        foreach ($color_map as $post_key => $db_column) {
            ...
            $color_value = htmlspecialchars($_POST[$post_key]);
            
            if (!in_array($color_value, $brickcolor)) {
                $valid_request = false;
                break;
            }

            $update_data[$db_column] = $color_value;
        }
        ...
        $stmt->bindValue(':id', $currentuid, SQLITE3_INTEGER);
        $stmt->execute();
    } else {
        echo "No color data received. Weird.";
    }
}
